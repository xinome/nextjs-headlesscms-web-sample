...
- [x] `app/posts/[slug]/page.tsx` を新規作成
- [x] `params.slug` から記事を取得（`getPostBySlug(slug)` を利用、Next.js 15+ の Promise params に対応）
- [x] 記事が存在しなければ `notFound()` を呼び出す
- [x] 本文 `content`（リッチエディタHTML）を表示
- [x] アイキャッチ画像（`eyecatch`）がある場合のみ `next/image` で表示
- [x] 公開日・カテゴリ名などのメタ情報を表示
- [x] `export const revalidate = 60` を設定し、ISRを有効化

### SEO実装（generateMetadata）
- [x] `generateMetadata` を実装
- [x] `seoTitle` があればタイトルに使用、なければ `title` を使用
- [x] `seoDescription` があれば説明に使用、なければ `excerpt` から生成（なければ固定文）
- [x] OGPの `title` / `description` / `image` を最低限設定
- [x] `noindex === true` の場合、`robots` を noindex に設定

### SSG実装（generateStaticParams）
- [x] `generateStaticParams` を実装し、全 `posts` の `slug` をSSG対象として列挙
- [x] `getPosts({ fields: 'slug', limit: 100 })` を利用してslug一覧を取得

### 型・クエリの利用
- [x] `Post` 型（`types/microcms.ts`）を利用して型安全に実装
- [x] `lib/microcms/queries.ts` の `getPostBySlug` / `getPosts` を利用

### 動作確認
- [x] 実在する `slug` にアクセスして、詳細ページが表示されることを確認
- [x] 存在しない `slug` で `notFound()` が機能していることを確認
- [x] `<head>` 内の `title` / `meta description` / OGPタグが期待どおり設定されていることを確認
- [ ] `noindex` 設定の記事で `robots` が noindex になっていることを確認（実装済み、動作確認は任意）
- [ ] 本番モード（`npm run build && npm start`）でSSG/ISRが期待どおりに動作することを確認（実装済み、動作確認は任意）
...
## 完了条件
- [x] `/posts/[slug]` ページが実装されている
- [x] `getPostBySlug` による記事取得と `notFound()` が正しく動作している
- [x] 本文 `content` がリッチテキストとして表示されている
- [x] アイキャッチ画像が `next/image` で表示されている（ある場合）
- [x] `generateMetadata` によりタイトル / 説明 / OGP / robots(noindex) が期待どおり設定されている
- [x] `generateStaticParams` により全 `slug` がSSG対象になっている
- [x] 手動テストで要件を満たすことが確認されている

## 関連ファイル
- app/posts/[slug]/page.tsx ✅ 作成済み（Next.js 15+ の Promise params に対応）
